...
public interface TourneeMapper {
    ...
    TourneeDTO toDTO(TourneeEntity tourneeEntity);
    ...
    default List<String> mapLivraisons(List<LivraisonEntity> livraisons) {
        return livraisons.stream().map(LivraisonEntity::getReference).collect(Collectors.toList());
    }
}
